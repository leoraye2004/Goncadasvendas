...
if(isset($_POST['status'])){
    ...
    $update->execute([$status, $id_pedido]);
}

// Buscar itens
$comando = $pdo->prepare("
    SELECT pi.*, p.nome_produto
    FROM pedidos_itens pi
    JOIN produtos p ON pi.id_produto = p.id_produto
    WHERE pi.id_pedido = ?
");
$comando->execute([$id_pedido]);
$itens = $comando->fetchAll();
?>
...
<h3>Pedido Nº <?php echo $id_pedido; ?></h3>
...
<a href="adm_pedidos.php" class="d-flex col-1" style="margin-left: 10%; margin-top:-3.5%;">
